...
function apply(file,external){
    ...
    var path = $('#cur_dir').val(); /* TEW ADDED '/' TO SEE IF IT WOULD WORK...  NO DICE */   
    //path = path.replace('\\', '/');
    ...
    if($.inArray(ext, ext_img) > -1){
        fill='<img src="'+path+file+'" alt="'+alt_name+'" />'; /* TEW TRYING IT HERE  */
    }else{
...
function apply_any(path, file) {
	path = path.replace('\\', '/');
	parent.tinymce.activeEditor.windowManager.getParams().setUrl(path+file); /* TRYING HERE NO LUCK */
	parent.tinymce.activeEditor.windowManager.close();
	return false;	
}
